...
cnn.add(tf.keras.layers.Conv2D(filters=32, kernel_size=3, input_shape=[28, 28, 3]))  # 3 channels for RGB
...
def make_predictions_and_check_accuracy(image_folder, class_indices_rev):
    ...
    for image_path in image_paths:
        img = image.load_img(image_path, target_size=(28, 28))  # Loading as RGB
...
image_folder = '../datasets/mnist/single_prediction/'
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('cnn_mnist_e10_accuracy.png')
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('cnn_mnist_e10_loss.png')
